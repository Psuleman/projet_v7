...
const Caracteristique = () => {
	...
	const {tagsReferencementUpdate, categorieUpdate, categorieEnUpdate, sousCategorieUpdate, sousCategorieEnUpdate, couleurUpdate, couleurEnUpdate, paysOrigineUpdate, grilleTailleUpdate, attributUpdate, filtreUpdate, filtreEnUpdate} = useContext(ProduitContext)
	...
	return (
			<div className="caracteristique">
				...
				<div className="label_value">
					<div>TAGS : </div>
        			<div><textarea value={tagsReferencementUpdate}  disabled="disabled"/></div>  					
				</div>
...

				{
					filtreAutre && 
					<div>
						<TemplateInputSelect label="FILTRE" typeInput="text" valeurEnUpdate={produit.filtreEn} attributEnUpdate={filtreEnUpdate} setAttributEnUpdate={setFiltreEnUpdate} valeurUpdate={produit.filtre} attributUpdate={filtreUpdate} setAttributUpdate={setFiltreUpdate} urlAPI="http://212.129.3.31:8080/api/sous_categorie_refs" />
				...
				<div>
						{
							filtres?        			
							<select  value={filtreUpdate} onChange={(e)=>{setFiltreUpdate(e.target.value)}}>
								<option onClick={() => {
									setFiltreAutre(true)
								}}>Autres</option>
								{

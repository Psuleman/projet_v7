...
const TemplateInputSelect = ({children, label, typeInput, valeurUpdate, valeurEnUpdate, attributUpdate, setAttributUpdate, urlAPI, attributEnUpdate, setAttributEnUpdate}) => {
    ...
    const {entretienUpdate, setEntretienUpdate, entretienEnUpdate, setEntretienEnUpdate,
...
const handleChange = (motcle) => {
    ...
    if(label != "PRIX DE VENTE AVEC REMISE"){
        ...
        if(motcle != ""){   
            ...
            listValue.forEach(element => {
                ...
                if(label=="TAILLE PORTÉE PAR LE MANNEQUIN" && element.taille.match(regex))
                    tab.push(element)     
    ...
    return (
                        ...
                        {showList && valeurUpdate != attributUpdate && <div>            
                            {tabfiltre &&
                                tabfiltre.map((item)=>{
                                    ...
                                    if(label == "COULEUR EN"){
                                            return (
                                                <div className="item" key={item.couleurRefEn}
                                                onClick={()=>{ 
                                                    setValueModification(item.couleurRefEn)
                                                    setAttributUpdate(item.couleurRefEn)
                                                    setShowList(false)
                                                }}
                                                >{item.couleurRefEn}</div>
                                            )
                                            } ///if(label == "COULEUR")
                                    ...
                                    if(label == "TAILLE PORTÉE PAR LE MANNEQUIN"){
                                            return (
                                                <div className="item" key={item.taille}
                                                onClick={()=>{ 
                                                    setValueModification(item.taille)
                                                    setAttributUpdate(item.taille)
                                                    setShowList(false)
                                                }}
                                                >{item.taille}</div>
                                            )
                                    ...
                                    if(label == "FILTRE"){
                                            return (
                                                ...
                                                onClick={()=>{
                                                    setValueEn(item.filtreEn) 
                                                    setAttributEnUpdate(item.filtreEn)
        ...
        {
            (label=="UNIVERS" || label=="CATEGORIE" || label=="SOUS CATEGORIE" || label=="FILTRE" || label=="COULEUR" || label=="COUPE" || label=="Entretien") && 
            <div className="label_value">
